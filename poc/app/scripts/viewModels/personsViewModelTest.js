...
var personStateChangedHandler = function (context) {
	return function(oldState, newState){
		...
		nextState.collection = nextState.collection.map(function(person){
			if(person.id === oldState.id){
				...
				nextState.selected = context.Person(personNextState);
				return nextState.selected;
			}
			return person;
		});
		context.raiseStateChanged(context.appState, nextState);
	};
}
MyApp.PersonsViewModel = (function(IMVVM){ 
	var PersonsViewModel = IMVVM.createViewModel(DataContext, personStateChangedHandler);
